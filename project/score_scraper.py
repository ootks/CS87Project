...
num_posts = 1000
score_threshold = 10
score_post_demarcator = '\x0b' # vertical tab
scraped_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),'data/scores')
    ...
    with open(os.path.join(scraped_path, 'top_%d_posts.txt' % num_posts), 'w') as f:
        for submission in subreddit.top(limit=num_posts):
            ...
            for comment in submission.comments.list():
                ...
                line += score_post_demarcator
                line += str(comment.score)
                line += '\n'
                f.write(line)
                num_comments += 1
